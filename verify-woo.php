...
function h(string $s): void { line(); line("### " . $s); }
function ok(string $s): void { line("  ✓ " . $s); }
function warn(string $s): void { line("  ⚠ " . $s); }
...
foreach ($sampleOrders as $o) {
    ...
    if ($cid === 0) { $guest++; } else { $registered++; }

    $phone = trim((string) ($o['billing']['phone'] ?? ''));
    if ($phone === '') { $noPhone++; } else { $withPhone++; }
}
...
if ($prod['status'] === 200 && is_array($prod['body'])) {
    ...
    $checked = 0; $varNoSku = 0; $varTotal = 0;
