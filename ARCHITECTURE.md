...
2026-09-25) — this file is now an index only. **New-task logging convention (CLAUDE.md §14
...
- **GATE 3 (SQL-injection test on RuleCompiler) / GATE 4 (AI connection has no write access):** not yet reached — Segments/AI modules start Sprint 5+ (see `sprint/5-segmentation` branch, not yet merged to `main`; out of scope for this index, which only covers what is on `main`).
