...
- **`products.sku`/`products.price` (nullable, deviates from PRD §09's literal `products` schema) — project-owner decision, Sprint 6:** PRD §09 defines no `sku`/`price` column on `products` (a "simple" WooCommerce product's own identifiers), and P2-05 explicitly declined to invent one. This left every simple-product order line permanently unresolvable (confirmed 0% on dev). The project owner decided to add these two columns (new, reversible migration) plus a new resolver step (`CatalogService::resolveProductBySku()`, PRD §10 step "3.5") rather than a synthetic default-variation row. `docs/architecture/sprint-6.md` ("رفع" section).
...
- ⚠ **Infrastructure fixed and shipped (decisions 1 & 2 below); dev data still 0% — decision 3 stopped on one new finding, not a 429/5xx (`docs/architecture/sprint-6.md`, "رفع" section):** the root cause (diagnosed earlier, see history below) had three parts. **(1) Simple-product resolvability — FIXED, shipped:** `products.sku`/`price` columns added (new migration, deviates from PRD §09's literal schema — an explicit project-owner decision, not invented; `ProductDto`/`ProductMapper` already read these from Woo, they were just dropped before `products` had anywhere to put them), plus a new resolver step (`CatalogService::resolveProductBySku()`) and cross-table SKU-uniqueness guards. **(2) No catalog sync entry point — FIXED, shipped:** `SyncEntity::Catalog`, `CatalogSyncJob` (thin, calls the already-built P2-05 `CatalogSyncService`), `hm:sync --entity=catalog`, a standalone nightly schedule (`01:30` Tehran — the full PRD §22 chain is P6-09, not yet built). `SyncService::run()` now explicitly rejects `SyncEntity::Catalog` so Orders' cursor/window machinery can never be misapplied to it. **(3) Actually running it live — STOPPED:** the real run hit a genuine, pre-existing, already-documented-as-accepted risk (P2-05's own log: category `name`/`slug` overflow is "deliberately not truncated"), not the 429/5xx case flagged for stopping, but a real, new blocker all the same — Woo category id=2346 has a 194-char slug against the `product_categories.slug varchar(180)` column (1 of 173 real categories; DB transaction rolled back cleanly, nothing written). Needs one more small decision (truncate like other Sync fields / widen the column / skip-and-warn) before the live sync can complete. Until then: **dev's real order_items→product resolution rate is still 0.00% (0/61,358)**, `customer_product_purchases`/`customer_category_purchases` still 0 rows — the fix is ready, the dev data isn't populated yet.
...
| Diagnosis      | sprint-6.md | Root cause of 0% order_items→product resolution on dev fully diagnosed; NOT fixed (stopped per instruction — needs a human decision + live Woo sync) |
| Bugfix         | sprint-6.md | products.sku/price + resolver step + catalog sync Job/Command shipped (decisions 1-2); live run (decision 3) stopped on a real, pre-existing category-slug-length finding — dev still 0% |
