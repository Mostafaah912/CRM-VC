...
- ⚠ **ROOT CAUSE NOW FULLY DIAGNOSED (`docs/architecture/sprint-6.md`, "تشخیص" section) — two separate, additive causes, neither fixed (stopped per explicit instruction: the real fix needs live Woo execution, decision pending):**
  1. **Primary (100% of the 0-resolution rate): the real Woo catalog has never been synced to dev at all.** `products`/`product_variations` on dev are 100% `DemoDataSeeder` fixture rows (`woo_product_id` 3000-3011, `synced_at` pinned to `2026-06-30 08:30:00`), sharing **zero** SKU overlap with the 61,358 real `order_items` (1,911 real distinct SKUs like `hmp-7614`). `sync_cursors` only ever had an `orders` row. `SyncEntity` enum supports only `Orders` (its own comment: "catalog and customers are not run through here yet"); `hm:sync --entity=products` fails with a fixed "Unknown entity" message. `CatalogSyncService` (built P2-05, 1036 green tests) is called from **nowhere** in the app (`grep -rln CatalogSyncService app/ routes/` returns only its own file) — P2-05/P2-08/P2-10 all explicitly documented this as *not built* (no Job/Cursor/Window for catalog), not a bug.
  2. **Secondary, structural: a "simple" (non-variable) Woo product can never resolve even after a real sync**, because `products` has no `sku`/`price` column and the 4-step resolver only ever matches `product_variations`. This exact decision ("does a simple product get a synthetic default variation?") was flagged as blocking *before* P2-06 (`sprint-2.md` line 77/93) and was never made.
  3. **Backfill ceiling for whichever fix is chosen:** `order_items` never persisted Woo's raw `product_id`/`variation_id` — only the (currently all-null) resolved FK columns survive alongside `sku` (90.55%, 55,560/61,358 populated) and `woo_item_id`. A local backfill can only ever reach ~90.55% via SKU re-matching; the SKU-less 9.45% needs a fresh Woo re-sync (plus a new migration to keep the raw ids next time) to ever close.

    Fix plan (3 ordered decisions, none executed): (1) human decision — synthetic default variation vs. `products.sku`/`price` columns; (2) safe to build now, no live calls — wire `SyncEntity::Categories/Products/Variations` + a thin Job into `hm:sync`/scheduler, mirroring Orders' P2-08 pattern exactly, TEST FIRST with `FakeWooClient`; (3) **requires explicit go-ahead** — actually run that sync against the live store on dev; after that, no separate backfill code is needed since `BuildCustomerPurchaseAggregatesJob` (P6-01) already rebuilds from `order_items.product_id` idempotently. `docs/architecture/sprint-2.md` (P2-05/P2-06 origin), `docs/architecture/sprint-6.md` (P6-01 first measured the 0%, this task diagnosed why).
